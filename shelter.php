...
function addNote($link){
    if(isset($_POST['text']) and isset($_POST['contacts'])){
       ...
       $query = "SELECT position, user_id, id  FROM advert WHERE position = (SELECT MAX(position) FROM advert )"; //Достаю максимальную позицию
        ...
        $query="INSERT INTO advert (text, user_id, contacts, issue, position) VALUES ('$text', '$id', '$contacts', '1', '$topPosition')";
...
function getList($link){
    ...
    $query = "SELECT text, position, contacts, status, users.id , advert.id as adId, name, phone_numb, email FROM users
    RIGHT JOIN advert ON users.id = advert.user_id WHERE issue = '1' ORDER BY position DESC LIMIT $referencePoint, $notesOnPage ";
    $result = mysqli_query($link, $query) or die(mysqli_error($link));
    $content = '';
    for($arr = []; $step = mysqli_fetch_assoc($result); $arr[] = $step);
        ...
        $content.="
            <table>
                ...
                if($userId == $adUserId AND $userRole != 'guest'){
                    ...
                    $content.="<tr>
                                    <td><form method='GET'>
                                    <button><a href=\"shelter.php?position=$adPosition&ad_id=$adId\">Top up</a></button>
                                    </form></td>
                                </tr>";
                }
